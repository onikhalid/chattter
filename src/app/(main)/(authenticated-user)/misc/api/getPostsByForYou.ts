...
type QueryKey = ['all-interests-posts', { interests: string[] }];
...
const getPosts = async ({ pageParam = null, queryKey }: QueryFunctionContext<QueryKey, QueryDocumentSnapshot<DocumentData> | null>): Promise<QueryResult> => {
  const [, { interests }] = queryKey; 
  const postsCollectionRef = collection(db, "posts");
  let q = query(
    postsCollectionRef,
    where("tags", "array-contains-any", interests.length > 0 ? interests : [""]),
...
const useInterestsPostsInfiniteQuery = (interests: string[]) => {
  ...
  return useInfiniteQuery<QueryResult, Error, InfiniteData<QueryResult>, QueryKey, QueryDocumentSnapshot<DocumentData> | null>({
    queryKey: ['all-interests-posts', { interests }],
    queryFn: getPosts,
...
export default useInterestsPostsInfiniteQuery;
